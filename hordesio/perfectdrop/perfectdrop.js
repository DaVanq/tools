window.onload = function(){
	...
	document.getElementById("item-selector").addEventListener("change", function(e){
		...
		for (var i in statNames) {
			...
			chance*=Math.round((stats.float)?((level*(stats.multi*stats.high)+stats.base)-(level*(stats.multi*stats.low)+stats.base)).toFixed(1)/0.1:Math.ceil((level*(stats.multi*stats.high)+stats.base)-(level*(stats.multi*stats.low)+stats.base)))
		}
